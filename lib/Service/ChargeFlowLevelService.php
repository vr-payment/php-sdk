...
class ChargeFlowLevelService {
	 ...
	 * @throws \VRPay\Sdk\VersioningException
	 * @throws \VRPay\Sdk\Http\ConnectionException
	 * @return \VRPay\Sdk\Model\ChargeFlowLevel
	 */
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\VRPay\Sdk\Model\ChargeFlowLevel',
				'/charge-flow-level/read'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\VRPay\Sdk\Model\ChargeFlowLevel', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \VRPay\Sdk\Model\EntityQuery $query The query restricts the payment flow levels which are returned by the search. (required)
	 * @throws \VRPay\Sdk\ApiException
	 * @throws \VRPay\Sdk\VersioningException
	 * @throws \VRPay\Sdk\Http\ConnectionException
	 * @return \VRPay\Sdk\Model\ChargeFlowLevel[]
