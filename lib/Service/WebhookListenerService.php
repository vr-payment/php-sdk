...
class WebhookListenerService {
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \VRPay\Sdk\Model\WebhookListenerCreate $entity The webhook listener object with the properties which should be created. (required)
	 * @throws \VRPay\Sdk\ApiException
	 * @throws \VRPay\Sdk\VersioningException
	 * @throws \VRPay\Sdk\Http\ConnectionException
	 * @return \VRPay\Sdk\Model\WebhookListener
	 */
	...
	public function createWithHttpInfo($space_id, $entity) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\VRPay\Sdk\Model\WebhookListener',
				'/webhook-listener/create'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\VRPay\Sdk\Model\WebhookListener', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \VRPay\Sdk\Model\EntityQuery $query The query restricts the webhook listeners which are returned by the search. (required)
	 * @throws \VRPay\Sdk\ApiException
	 * @throws \VRPay\Sdk\VersioningException
	 * @throws \VRPay\Sdk\Http\ConnectionException
	 * @return \VRPay\Sdk\Model\WebhookListener[]
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \VRPay\Sdk\Model\WebhookListenerUpdate $entity The webhook listener object with all the properties which should be updated. The id and the version are required properties. (required)
	 * @throws \VRPay\Sdk\ApiException
	 * @throws \VRPay\Sdk\VersioningException
	 * @throws \VRPay\Sdk\Http\ConnectionException
	 * @return \VRPay\Sdk\Model\WebhookListener
